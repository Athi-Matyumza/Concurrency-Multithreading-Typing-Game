...
//shared class variables
	...
	static FallingWord[] wordsH; //NEW
	static WordMover[] wrdShft;
	static HungryWordMover[] wrdShftH;//NEW
	...
	public static void setupGUI(int frameX,int frameY,int yLimit) {
		...
		gameWindow = new GamePanel(words, wordsH,yLimit,done,started,won);
		...
		quitB.addActionListener(new ActionListener() {
				  public void actionPerformed(ActionEvent e) {
					 ...
					 //word movers waiting on starting line
					  ...
					  try {
						  wrdShftH[0].join(); //STOPS THE THREAD WITH MULTIPLE WORDS
	...
	public static void createWordMoverThreads() {
		...
		wordsH[0] = new FallingWord(dict.getNewWord(), 0, frameX,yLimit , "Hori"); //MODIFY TO GET NEW WORDS - UNDER CONSTRUCTION

		for (int i=0;i<noWords;i++) {
			words[i]=new FallingWord(dict.getNewWord(),gameWindow.getValidXpos(),yLimit, "Vert");
//			System.out.println(words[i].getWord() + " X=" + words[i].getX() + " Y=" + words[i].getY());
		}

		//create threads to move them
	    for (int i=0;i<noWords;i++) {
	    		wrdShft[i] = new WordMover(words[i], wordsH[0],dict,score,startLatch,done,pause);
				//CONSTRUCT - LET THE VERTICAL WORDS KNOW ABOUT THE HUNGRY WORDS' POSITION (for collisions)
	    }

		wrdShftH[0] = new HungryWordMover(wordsH[0], words, dict, score,startLatch,done,pause);

        //word movers waiting on starting line
     	for (int i=0;i<noWords;i++) {
     		wrdShft[i] .start();
     	}
		 wrdShftH[0].start(); // STARTING THE HORIZONTAL WORD SHIFTER
	}
	...
	public static void main(String[] args) {
		...
		totalWords=24;
		noWords=8; // controls how many words appear on screen
		...
		wordsH = new FallingWord[1];//NEW - MODIFY SIZE TO TAKE IN MORE WORDS, ALL WORDS IN THE DICTIONARY/totalWords
		wrdShftH = new HungryWordMover[1];//NEW
		
		CatchWord.setWords(words, wordsH);  //class setter - static method
